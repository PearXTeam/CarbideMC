package ru.pearx.carbide.mc.common.caps.animation;
...
public class AnimationStateManager implements IAnimationStateManager
{
    ...
    public void changeState(int element, int index)
    {
        ...
        {
            CarbideMC.NETWORK.sendToAllAround(new CPacketSyncASMState(tile.getPos(), element, index), new NetworkRegistry.TargetPoint(tile.getWorld().provider.getDimension(), tile.getPos().getX(), tile.getPos().getY(), tile.getPos().getZ(), 256));
        }
    }
